...
import { Component, OnInit, ChangeDetectionStrategy, Input, ViewChild, Output, EventEmitter, OnChanges, OnDestroy } from '@angular/core';
import { MatTableDataSource, MatTable } from '@angular/material/table';

import { MatPaginator } from '@angular/material/paginator';
import { MatSort } from '@angular/material/sort';
import { IUsersList, UsersService } from 'ml-setup/shared/services/user-access-rights/users.service';
    ...
    */
    ...
    customFilterPredicate(data: any, filters: any): boolean {
        ...
        for (let i = 0; i < filters.length; i++) {
            ...
            if (filters[i].column === "departmentAccess" && filters[i].value !== "") {
                const tableData = data[filters[i].column];
                if (tableData.length > 0) {
                    tableData.map(res => {
                        if (res.departmentCode.trim().toLowerCase() === filters[i].value) {
            ...
            } else if (filters[i].column !== "loginHistory" && filters[i].column !== "functionAccess" && filters[i].column !== "departmentAccess") {
                const tableData = (data[filters[i].column]) ? data[filters[i].column].trim().toLowerCase() : data[filters[i].column];

                const fitsThisFilter = (tableData) ? tableData.includes(filters[i].value) : "";
                ...
                switch (propName) {
                    case 'searchFilters': {
                        ...
                        if (this.searchFilters !== undefined) {
                            ...
                            this.generateUserTable(1, this.searchFilters, undefined);
                        }
                        break;
                    }
                    case 'reloadList': {
    ...
    onRemove(usersList: any) {
        this.showLoader = true;
        this.usersService.deleteUser(usersList.id)
        .pipe(takeUntil(this._unsubscribeAll))
            .subscribe(
                () => {
    ...
    changeActive(event: any, element: any) {
        ...
        this.showLoader = true;
        this.usersService.updateUserStatus(element.id, isChecked)
        .pipe(takeUntil(this._unsubscribeAll))
            .subscribe(res => {
