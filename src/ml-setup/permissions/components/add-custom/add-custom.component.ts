...
import { Component, OnInit, ChangeDetectionStrategy, ViewChild, Input, SimpleChanges, Inject } from '@angular/core';
...
export class AddCustomComponent implements OnInit {
    ...
    constructor(
        ...
        if (modalParams && modalParams.hasOwnProperty('key') && modalParams.key.colId) {
    ...
    createColumn(resultObj) {
        this.permissionService.postPermission(resultObj)
            .subscribe(
                (res) => {
