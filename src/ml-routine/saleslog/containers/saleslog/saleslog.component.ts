      ...
      this.salesData.rowData.row.forEach(element => {
        ...
        if(moment(this.cellData[0]['"OD"']).isBefore(moment().format('DD-MMM-YY'),'months')){
          ...
          if(time){
            ...
            this.cellData[0]['type']='carryOver';
          }else{
            this.cellData[0]['type']='sold';
        ...
        if( this.cellData[0]['"AD"'] !== ""){
          this.cellData[0]['type']='delivered';
  ...
  filterGrid(option){
    ...
    // If filter matches change active state of columns*/ 
      if(option === 0){
        ...
        let row = this.rowResponse.filter(res=> res.type ==="carryOver");
        console.log(this.rowResponse)
        if(row.length >0){
          this.rowData = row;  
        ...
        let row = this.rowResponse.filter(res=> res.type ==="sold");
        ...
        let row = this.rowResponse.filter(res=> res.type ==="covered");
        ...
        let row = this.rowResponse.filter(res=> res.type ==="delivered");
