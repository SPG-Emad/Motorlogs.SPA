  ...
  generateGrid(months?, date?){
    ...
    if(this.routineSelected !== 3){
      ...
      department = department.find(el=>{
        ...
        return  Number(el.departmentId) === Number(this.decryptedDepartmentId);
      });
      ...
      department.map(res=>{
        this.departmentIDs.push({value: res.departmentId, viewValue: res.departmentName})
      })
      let count = 0;
      let ID=  0;
      department = department.find(el=>{
    ...
    .subscribe(res=>{
      ...
      if(this.routineSelected === 1){
        ...
        this.deliveredAvg = (this.totalRows > 0)? Number(this.vehicleProfit)/Number(this.totalRows):0;

        this.processedAvg = (this.processedUnits > 0)? Number(this.processedAmount)/Number(this.processedUnits):0;

        this.varianceAmount = this.processedAmount - this.vehicleProfit;
        this.varianceAvg = (this.totalRows > 0)? (this.varianceAmount)/this.totalRows: 0;

        this.totalAmount = Number(this.processedAmount) + Number(this.vehicleProfit);
        this.totalAvg = (this.totalRows > 0)? (this.totalAmount)/Number(this.totalRows):0;
        ...
        this.tradeinAvg = (this.tradeinUnits > 0)? Number(this.tradeinAmount)/Number(this.tradeinUnits):0;
  ...
  gridFilter(months?, date?, searchValue?){
    ...
    this.saleslog.fetchAllRows(obj).subscribe(res=>{
      ...
      if(this.routineSelected === 1){
        ...
        this.varianceAmount = this.processedAmount - this.vehicleProfit;
        this.varianceAvg = (this.totalUnits > 0)? (this.varianceAmount)/this.totalRows: 0;

        this.totalAmount = Number(this.processedAmount) + Number(this.vehicleProfit);
        this.totalAvg = (this.totalUnits > 0)? (this.totalAmount)/Number(this.totalRows):0;

        console.log(this.totalAmount, this.processedAmount, this.vehicleProfit);
